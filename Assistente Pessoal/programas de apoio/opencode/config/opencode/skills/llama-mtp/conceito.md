# Conceito & Persona — llama-mtp (Ontology R77)
...
# Helenização R77 — llama-mtp: MTP llama.cpp

## 1. Ontologia (o que É — escopo imutável)
- **Skill `llama-mtp`** helenizada de `https://github.com/ggml-org/llama.cpp` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: MTP llama.cpp — conceitos: MTP, speculative decoding, llama.cpp.
...
<system>
You are a specialist in MTP llama.cpp via skill `llama-mtp`.
...
<instruction>
Tarefa: aplicar padrão `llama-mtp` (inferência) ao contexto do harness.
...
- **Aceita**: MTP, speculative decoding, llama.cpp, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `llama-mtp` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/ggml-org/llama.cpp` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`inferência`), descartar ruído.
...
> Nota: helenização de `llama-mtp` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
