#!/usr/bin/env python3
"""
Mecanica — world-model-optimizer
R77 tríplice + R81 constrained decoding + R82 estrangulamento
Skill: world-model-optimizer — otimizador world model
...
class Output(BaseModel):
    """Output estrito de world-model-optimizer — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="world-model-optimizer", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-world-model-optimizer", "confidence": 0.95, "skill": "world-model-optimizer"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "world-model-optimizer", "extra": "lixo"}')
