# Conceito & Persona — openship (Ontology R77)

---
name: openship
description: |
  Self-hosted deployment platform. (absorvido de oblien/openship)
category: skill-tecnica
role: openship
type: skill
version: 1.0.0
origin: https://github.com/oblien/openship
---

# Helenização R77 — openship: openship

## 1. Ontologia (o que É — escopo imutável)
- **Skill `openship`** helenizada de `https://github.com/oblien/openship` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: openship — conceitos: ship, deploy.
...
<system>
You are a specialist in openship via skill `openship`.
...
<instruction>
Tarefa: aplicar padrão `openship` (deploy) ao contexto do harness.
...
- **Aceita**: ship, deploy, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `openship` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/oblien/openship` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`deploy`), descartar ruído.
...
> Nota: helenização de `openship` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
