...
> **⚠️ DRIFT 2026-09-18** — slots down (:9088/:9090/:9092/:9086, health 000); **único vivo :9084 (ingestor)** — fallback `base_url` do script (`http://127.0.0.1:9084`), **sem reatribuição de papel** (doutrina: slot caiu = redflag, nunca substituir ator). `check_slots()` emite REDFLAG por papel down no stderr e no relatório final (`redflags: [...]`). Override de transporte: env `A2A_BASE_URL` ou flag `--base-url`.
...
| 🏛️ Escalação | gemma-2-2b-it | :9092 | Suprema Corte — APENAS em impasse final (raro) |
...
- **Transporte** (2026-09-18): `base_url` default `http://127.0.0.1:9084` (fallback provisório — único slot vivo); override: env `A2A_BASE_URL` ou `--base-url`. `check_slots()` no início do loop: GET /health por papel (timeout 2s); slot down ⇒ REDFLAG no stderr + relatório final. Nunca reatribui papel.
- **Sampling** (R61/R77): propositor 0.6 · refutador 0.8 · escalação 0.8.
...
a2a_brainstorm:
  ...
  redflags: [...]          # slots down por papel (check_slots) — nunca silencioso
