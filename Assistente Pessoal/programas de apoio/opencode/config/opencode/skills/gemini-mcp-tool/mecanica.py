...
class Output(BaseModel):
    """Output estrito de gemini-mcp-tool — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="gemini-mcp-tool", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-gemini-mcp-tool", "confidence": 0.95, "skill": "gemini-mcp-tool"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "gemini-mcp-tool", "extra": "lixo"}')
