#!/usr/bin/env python3
"""
Mecanica — ai-agent-book
R77 tríplice + R81 constrained decoding + R82 estrangulamento
Skill: ai-agent-book — livro de padrões de agentes
...
class Output(BaseModel):
    """Output estrito de ai-agent-book — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="ai-agent-book", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-ai-agent-book", "confidence": 0.95, "skill": "ai-agent-book"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "ai-agent-book", "extra": "lixo"}')
