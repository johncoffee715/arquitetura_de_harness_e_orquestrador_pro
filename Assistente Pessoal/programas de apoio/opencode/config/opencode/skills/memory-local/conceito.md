# Conceito & Persona — memory-local (Ontology R77)
...
# Helenização R77 — memory-local: memória local mem0

## 1. Ontologia (o que É — escopo imutável)
- **Skill `memory-local`** helenizada de `https://github.com/memory-local` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: memória local mem0 — conceitos: mem0, ADD/UPDATE/DELETE, vetorial.
...
<system>
You are a specialist in memória local mem0 via skill `memory-local`.
...
<instruction>
Tarefa: aplicar padrão `memory-local` (memória persistente) ao contexto do harness.
...
- **Aceita**: mem0, ADD/UPDATE/DELETE, vetorial, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `memory-local` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/memory-local` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`memória persistente`), descartar ruído.
...
> Nota: helenização de `memory-local` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
