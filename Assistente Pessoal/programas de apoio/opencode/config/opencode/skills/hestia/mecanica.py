...
Skill: hestia — Héstia conformidade
...
class Output(BaseModel):
    """Output estrito de hestia — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="hestia", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-hestia", "confidence": 0.95, "skill": "hestia"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "hestia", "extra": "lixo"}')
