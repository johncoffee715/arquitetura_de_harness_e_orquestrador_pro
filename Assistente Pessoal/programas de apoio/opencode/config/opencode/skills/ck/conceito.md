# Conceito & Persona — ck (Ontology R77)
...
type: skill
version: 1.0.0
origin: https://github.com/ck
---

# Helenização R77 — ck: CK framework

## 1. Ontologia (o que É — escopo imutável)
- **Skill `ck`** helenizada de `https://github.com/ck` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: CK framework — conceitos: CK, benchmark, otimização.
...
<system>
You are a specialist in CK framework via skill `ck`.
...
<instruction>
Tarefa: aplicar padrão `ck` (benchmark) ao contexto do harness.
...
- **Aceita**: CK, benchmark, otimização, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `ck` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/ck` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`benchmark`), descartar ruído.
...
> Nota: helenização de `ck` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
