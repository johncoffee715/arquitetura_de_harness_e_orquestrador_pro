#!/usr/bin/env python3
"""
Mecanica — better-harness
R77 tríplice + R81 constrained decoding + R82 estrangulamento
Skill: better-harness — melhoria de harness
Origin: desconhecido
...
class Output(BaseModel):
    """Output estrito de better-harness — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="better-harness", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-better-harness", "confidence": 0.95, "skill": "better-harness"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "better-harness", "extra": "lixo"}')
