...
Skill: colibri — colibri framework
...
class Output(BaseModel):
    """Output estrito de colibri — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="colibri", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-colibri", "confidence": 0.95, "skill": "colibri"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "colibri", "extra": "lixo"}')
