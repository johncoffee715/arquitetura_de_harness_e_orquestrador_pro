# Conceito & Persona — spec-kit (Ontology R77)

---
name: spec-kit
description: |
  Spec-Driven Development toolkit (github): specify CLI (inspect/doctor/promote), bundles role-based, presets — define WHAT antes de HOW com qualquer agente. (absorvido de github/spec-kit)
...
# Helenização R77 — spec-kit: Spec Kit

## 1. Ontologia (o que É — escopo imutável)
- **Skill `spec-kit`** helenizada de `https://github.com/github/spec-kit` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: Spec Kit — conceitos: spec, kit.
...
<system>
You are a specialist in Spec Kit via skill `spec-kit`.
...
<instruction>
Tarefa: aplicar padrão `spec-kit` (especificação) ao contexto do harness.
...
- **Aceita**: spec, kit, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `spec-kit` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/github/spec-kit` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`especificação`), descartar ruído.
...
> Nota: helenização de `spec-kit` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
