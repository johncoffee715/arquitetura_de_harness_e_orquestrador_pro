# Conceito & Persona — orca (Ontology R77)
...
# Helenização R77 — orca: Orca

## 1. Ontologia (o que É — escopo imutável)
- **Skill `orca`** helenizada de `https://github.com/stablyai/orca` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: Orca — conceitos: orca, modelo.
...
<system>
You are a specialist in Orca via skill `orca`.
...
<instruction>
Tarefa: aplicar padrão `orca` (modelo) ao contexto do harness.
...
- **Aceita**: orca, modelo, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `orca` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/stablyai/orca` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`modelo`), descartar ruído.
...
> Nota: helenização de `orca` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
