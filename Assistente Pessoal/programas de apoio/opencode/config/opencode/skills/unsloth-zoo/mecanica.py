...
class Output(BaseModel):
    """Output estrito de unsloth-zoo — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="unsloth-zoo", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-unsloth-zoo", "confidence": 0.95, "skill": "unsloth-zoo"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "unsloth-zoo", "extra": "lixo"}')
