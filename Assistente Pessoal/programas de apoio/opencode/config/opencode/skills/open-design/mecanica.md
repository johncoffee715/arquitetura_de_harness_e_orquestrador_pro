# open-design — Mecânica de Ignição (R77 camada 3)

## 1. Seleção de motor (catálogo R75 — sempre refutando)

- **Categoria alvo**: `contrato-plano` (brief + artefato)
- **Slot esperado**: `:9088`
- **Refutação do catálogo**: design exige direção + iteração, não velocidade — aceito
...
1. Validar gabarito (deny) antes de qualquer ação — sem `brief`+`artifact` não há ciclo
2. Resolver motor via inventário (R75)
3. Brief → direção (+DESIGN.md) → artefato → preview → export
4. Gate categórico (R28): artefato real; nota R34 com bugs concretos

## 4. Funções focadas (Python, blocos curtos)

```python
def ignicao(brief: str, artifact: str) -> dict:
    """Brief + artefato. Retorna dict."""
