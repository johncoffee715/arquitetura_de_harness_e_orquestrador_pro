#!/usr/bin/env python3
"""
Mecanica — agent-reach
R77 tríplice + R81 constrained decoding + R82 estrangulamento
Skill: agent-reach — roteador multi-plataforma
...
class Output(BaseModel):
    """Output estrito de agent-reach — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="agent-reach", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-agent-reach", "confidence": 0.95, "skill": "agent-reach"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "agent-reach", "extra": "lixo"}')
