# Conceito & Persona — openwork-mcp (Ontology R77)
...
# Helenização R77 — openwork-mcp: MCP OpenWork

## 1. Ontologia (o que É — escopo imutável)
- **Skill `openwork-mcp`** helenizada de `https://github.com/different-ai/openwork` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: MCP OpenWork — conceitos: MCP, OpenWork.
...
<system>
You are a specialist in MCP OpenWork via skill `openwork-mcp`.
...
<instruction>
Tarefa: aplicar padrão `openwork-mcp` (MCP) ao contexto do harness.
...
- **Aceita**: MCP, OpenWork, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `openwork-mcp` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/different-ai/openwork` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`MCP`), descartar ruído.
...
> Nota: helenização de `openwork-mcp` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
