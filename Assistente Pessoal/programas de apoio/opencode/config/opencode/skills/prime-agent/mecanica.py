...
class Output(BaseModel):
    """Output estrito de prime-agent — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="prime-agent", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-prime-agent", "confidence": 0.95, "skill": "prime-agent"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "prime-agent", "extra": "lixo"}')
