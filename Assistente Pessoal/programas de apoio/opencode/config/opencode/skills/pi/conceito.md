# Conceito & Persona — pi (Ontology R77)

---
name: pi
description: |
  AI agent toolkit: unified LLM API, agent loop, TUI, coding agent CLI. (absorvido de earendil-works/pi)
...
# Helenização R77 — pi: PI

## 1. Ontologia (o que É — escopo imutável)
- **Skill `pi`** helenizada de `https://github.com/earendil-works/pi` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: PI — conceitos: pi, assistente.
...
<system>
You are a specialist in PI via skill `pi`.
...
<instruction>
Tarefa: aplicar padrão `pi` (assistente) ao contexto do harness.
...
- **Aceita**: pi, assistente, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `pi` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/earendil-works/pi` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`assistente`), descartar ruído.
...
> Nota: helenização de `pi` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
