...
- **R60-v3 — LIMIAR OPERACIONAL POR CURVA-DE-ESTABILIDADE (descontinua R60 original, 2026-08-25)**: a R60 ("131072 FIXO") está **DESCONTINUADA** — calculava KV não-quantizado e confundia teto físico de VRAM com limite cognitivo. Evidências: (1) header GGUF qwen35/ornith: `rope.freq_base` nativo = **10M** e `context_length` nativo = 262144 → aplicar `--rope-scale`/`--rope-freq-base` manual é **REFUTADO** (geometria nunca treinada; freq-base 1M distorceria posições); (2) fenômeno real registrado: Ornith-Q4 degrada/alucina ~146K (24/08) — causa provável: erro acumulado de KV-quant + atenção de longa distância, NÃO RoPE. Nova doutrina: o limite operacional do slot = ponto de estabilidade medido por probe needle-in-haystack (`recall por profundidade 5/25/50/75/95% + detector de invenção`, temp 0), NUNCA o máximo alocável; orquestrador evacua/comprime contexto ANTES do limiar medido. Física de serving segue crivo R66; bench Vulkan 25/08: sem `-fa` = 80 t/s > com `-fa` = 72 t/s (fa NÃO acelera neste backend); ubatch grande piora (49 t/s). Sampling antiloop (t0.45 rp1.08 pp0.15 fp0.05) = perfil opcional para geração longa, fora do padrão agentic R61. Slot :8083 atual: Ornith-Q5_K_M · K/V=q5_0 · -c 163840 · probe da curva em curso (alvos 100/128/146/160K). Fonte: header GGUF lido + bench A/B/C + decisão usuário.
