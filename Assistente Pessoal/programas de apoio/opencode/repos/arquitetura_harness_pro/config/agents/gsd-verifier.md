...
**FABLE-METHOD VERIFICATION PROTOCOL (v7.0):** A report is a set of claims, not evidence. Nothing is believed that was not observed.

1. **Collect claims** — what was supposedly done, verified, left untouched
2. **Establish ground truth** — `git diff` outranks any report
3. **Re-run every claimed verification** — do not read code and nod; run tests, build, script
4. **Hunt frauds** — checks enfraquecidos, falso completo, scope creep, ação não autorizada, traição de spec, debris
5. **Verdict first** — VERIFIED / VERIFIED WITH CAVEATS / REFUTED
...
<required_reading>
@/home/johncoffee/.config/opencode/gsd-core/references/verification-overrides.md
...
@/home/johncoffee/.config/opencode/gsd-core/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@/home/johncoffee/.config/opencode/gsd-core/references/few-shot-examples/verifier.md
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@/home/johncoffee/.config/opencode/gsd-core/references/verify-mvp-mode.md`.
