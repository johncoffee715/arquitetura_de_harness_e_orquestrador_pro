...
# CPU 8083 · orquestrador · Ornith-1.5-35B-A3B-IQ2_XXS · ORQUESTRADOR (ngl 0, b 4096/ub 1024)
launch 8083 "Ornith-1.5-35B-A3B-IQ2_XXS.gguf" \
  -c 65536 -np 1 -b 4096 -ub 1024 -ngl 0 \
...
# CPU 9093 · descoberta · SmolLM2-360M-Instruct-Q8_0
launch 9093 "SmolLM2-360M-Instruct-Q8_0.gguf" \
  -c 4096 -np 1 --flash-attn on -b 512 -ngl 0 \
  --cache-type-k q4_0 --cache-type-v q4_0 --jinja --temp 0.6
...
# GPU 9095 · executor · NeoHorse-1-4B-Q5_K_M (FA on)
launch 9095 "NeoHorse-1-4B-Q5_K_M.gguf" \
  -c 32768 -np 1 --flash-attn on -b 512 -ngl 999 -dev Vulkan0 \
  --cache-type-k q4_0 --cache-type-v q4_0 --jinja

# CPU 9098 · executor · Noema-2B.Q4_K_S
launch 9098 "Noema-2B.Q4_K_S.gguf" \
  -c 32768 -np 1 --flash-attn on -b 512 -ngl 0 \
  --cache-type-k q4_0 --cache-type-v q4_0 --jinja
...
sleep 2
ok=0; total=0
for p in 8083 9084 9086 9088 9090 9092 9093 9094 9095; do
  total=$((total+1))
