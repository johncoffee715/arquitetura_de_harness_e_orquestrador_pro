# Conceito & Persona — world-model-optimizer (Ontology R77)
...
role: world-model-optimizer
type: skill
version: 1.0.0
origin: https://github.com/experientiallabs/world-model-optimizer
---

# Helenização R77 — world-model-optimizer: otimizador world model

## 1. Ontologia (o que É — escopo imutável)
- **Skill `world-model-optimizer`** helenizada de `https://github.com/experientiallabs/world-model-optimizer` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: otimizador world model — conceitos: world model, otimização.
...
<system>
You are a specialist in otimizador world model via skill `world-model-optimizer`.
...
<instruction>
Tarefa: aplicar padrão `world-model-optimizer` (otimização) ao contexto do harness.
...
- **Aceita**: world model, otimização, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `world-model-optimizer` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/experientiallabs/world-model-optimizer` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`otimização`), descartar ruído.
...
> Nota: helenização de `world-model-optimizer` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
