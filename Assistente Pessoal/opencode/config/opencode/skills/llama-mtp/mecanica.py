...
class Output(BaseModel):
    """Output estrito de llama-mtp — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="llama-mtp", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-llama-mtp", "confidence": 0.95, "skill": "llama-mtp"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "llama-mtp", "extra": "lixo"}')
