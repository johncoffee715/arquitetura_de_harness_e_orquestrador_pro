# Conceito & Persona — react-bits (Ontology R77)

---
name: react-bits
description: |
  >- Catálogo de componentes UI React reutilizáveis (helenizado de serif-ai/react-bits, 90+ componentes, 4 variantes por componente, MIT): animações, backgrounds, componentes, textos, agrupados por categoria. Use ao construir interfaces React, precisar de componente de animação/background/ texto pront
...
# Helenização R77 — react-bits: React Bits

## 1. Ontologia (o que É — escopo imutável)
- **Skill `react-bits`** helenizada de `https://github.com/react-bits` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: React Bits — conceitos: React, bits, UI.
...
<system>
You are a specialist in React Bits via skill `react-bits`.
...
<instruction>
Tarefa: aplicar padrão `react-bits` (UI) ao contexto do harness.
...
- **Aceita**: React, bits, UI, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `react-bits` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/react-bits` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`UI`), descartar ruído.
...
> Nota: helenização de `react-bits` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
