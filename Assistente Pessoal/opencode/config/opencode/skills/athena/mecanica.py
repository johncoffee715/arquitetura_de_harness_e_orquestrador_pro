...
Skill: athena — revisão arquitetural
...
class Output(BaseModel):
    """Output estrito de athena — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="athena", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-athena", "confidence": 0.95, "skill": "athena"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "athena", "extra": "lixo"}')
