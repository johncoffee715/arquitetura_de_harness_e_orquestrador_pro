# Conceito & Persona — recursive-llm (Ontology R77)
...
# Helenização R77 — recursive-llm: LLM recursivo

## 1. Ontologia (o que É — escopo imutável)
- **Skill `recursive-llm`** helenizada de `https://github.com/grishahq/recursive-llm` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: LLM recursivo — conceitos: recursão, LLM.
...
<system>
You are a specialist in LLM recursivo via skill `recursive-llm`.
...
<instruction>
Tarefa: aplicar padrão `recursive-llm` (recursivo) ao contexto do harness.
...
- **Aceita**: recursão, LLM, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `recursive-llm` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/grishahq/recursive-llm` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`recursivo`), descartar ruído.
...
> Nota: helenização de `recursive-llm` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
