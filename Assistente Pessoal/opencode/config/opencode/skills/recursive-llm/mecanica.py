...
class Output(BaseModel):
    """Output estrito de recursive-llm — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="recursive-llm", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-recursive-llm", "confidence": 0.95, "skill": "recursive-llm"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "recursive-llm", "extra": "lixo"}')
