...
class Output(BaseModel):
    """Output estrito de sentry-mcp — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="sentry-mcp", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-sentry-mcp", "confidence": 0.95, "skill": "sentry-mcp"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "sentry-mcp", "extra": "lixo"}')
