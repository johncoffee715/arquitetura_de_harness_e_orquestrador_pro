# Conceito & Persona — book-to-skill (Ontology R77)
...
# Helenização R77 — book-to-skill: livro para skill

## 1. Ontologia (o que É — escopo imutável)
- **Skill `book-to-skill`** helenizada de `https://github.com/book-to-skill` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: livro para skill — conceitos: livro→skill, destilação, helenização.
...
<system>
You are a specialist in livro para skill via skill `book-to-skill`.
...
<instruction>
Tarefa: aplicar padrão `book-to-skill` (conversão livro) ao contexto do harness.
...
- **Aceita**: livro→skill, destilação, helenização, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `book-to-skill` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/book-to-skill` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`conversão livro`), descartar ruído.
...
> Nota: helenização de `book-to-skill` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
