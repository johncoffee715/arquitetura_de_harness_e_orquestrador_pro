# Conceito & Persona — impeccable (Ontology R77)
...
type: skill
version: 1.0.0
origin: https://github.com/impeccable
---

# Helenização R77 — impeccable: código impecável

## 1. Ontologia (o que É — escopo imutável)
- **Skill `impeccable`** helenizada de `https://github.com/impeccable` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: código impecável — conceitos: qualidade, lint, 59 regras.
...
<system>
You are a specialist in código impecável via skill `impeccable`.
...
<instruction>
Tarefa: aplicar padrão `impeccable` (qualidade) ao contexto do harness.
...
- **Aceita**: qualidade, lint, 59 regras, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `impeccable` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/impeccable` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`qualidade`), descartar ruído.
...
> Nota: helenização de `impeccable` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
