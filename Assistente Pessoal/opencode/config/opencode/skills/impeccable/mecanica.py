...
Skill: impeccable — código impecável
...
class Output(BaseModel):
    """Output estrito de impeccable — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="impeccable", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-impeccable", "confidence": 0.95, "skill": "impeccable"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "impeccable", "extra": "lixo"}')
