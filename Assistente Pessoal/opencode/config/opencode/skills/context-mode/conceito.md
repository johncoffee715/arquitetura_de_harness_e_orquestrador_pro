# Conceito & Persona — context-mode (Ontology R77)
...
# Helenização R77 — context-mode: modo contexto

## 1. Ontologia (o que É — escopo imutável)
- **Skill `context-mode`** helenizada de `https://github.com/context-mode` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: modo contexto — conceitos: contexto, modo, seleção.
...
<system>
You are a specialist in modo contexto via skill `context-mode`.
...
<instruction>
Tarefa: aplicar padrão `context-mode` (gerenciamento) ao contexto do harness.
...
- **Aceita**: contexto, modo, seleção, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `context-mode` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/context-mode` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`gerenciamento`), descartar ruído.
...
> Nota: helenização de `context-mode` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
