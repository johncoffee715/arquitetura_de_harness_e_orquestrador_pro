#!/usr/bin/env python3
"""
Mecanica — vercel-agent-skills
R77 tríplice + R81 constrained decoding + R82 estrangulamento
Skill: vercel-agent-skills — Vercel Agent Skills
...
class Output(BaseModel):
    """Output estrito de vercel-agent-skills — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="vercel-agent-skills", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-vercel-agent-skills", "confidence": 0.95, "skill": "vercel-agent-skills"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "vercel-agent-skills", "extra": "lixo"}')
