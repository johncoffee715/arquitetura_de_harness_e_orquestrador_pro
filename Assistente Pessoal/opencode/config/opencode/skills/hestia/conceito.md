# Conceito & Persona — hestia (Ontology R77)
...
# Helenização R77 — hestia: Héstia conformidade

## 1. Ontologia (o que É — escopo imutável)
- **Skill `hestia`** helenizada de `https://github.com/hestia` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: Héstia conformidade — conceitos: conformidade, contrato, Héstia.
...
<system>
You are a specialist in Héstia conformidade via skill `hestia`.
...
<instruction>
Tarefa: aplicar padrão `hestia` (gate conformidade) ao contexto do harness.
...
- **Aceita**: conformidade, contrato, Héstia, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `hestia` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/hestia` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`gate conformidade`), descartar ruído.
...
> Nota: helenização de `hestia` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
