...
Skill: omniroute — gateway OmniRoute
...
class Output(BaseModel):
    """Output estrito de omniroute — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="omniroute", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-omniroute", "confidence": 0.95, "skill": "omniroute"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "omniroute", "extra": "lixo"}')
