# Conceito & Persona — deepagents (Ontology R77)
...
# Helenização R77 — deepagents: deep agents

## 1. Ontologia (o que É — escopo imutável)
- **Skill `deepagents`** helenizada de `https://github.com/langchain-ai/deepagents` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: deep agents — conceitos: agente profundo, hierarquia.
...
<system>
You are a specialist in deep agents via skill `deepagents`.
...
<instruction>
Tarefa: aplicar padrão `deepagents` (agentes) ao contexto do harness.
...
- **Aceita**: agente profundo, hierarquia, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `deepagents` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/langchain-ai/deepagents` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`agentes`), descartar ruído.
...
> Nota: helenização de `deepagents` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
