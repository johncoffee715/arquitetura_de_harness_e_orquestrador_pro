#!/usr/bin/env python3
"""
Mecanica — memory-local
R77 tríplice + R81 constrained decoding + R82 estrangulamento
Skill: memory-local — memória local mem0
...
class Output(BaseModel):
    """Output estrito de memory-local — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="memory-local", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-memory-local", "confidence": 0.95, "skill": "memory-local"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "memory-local", "extra": "lixo"}')
