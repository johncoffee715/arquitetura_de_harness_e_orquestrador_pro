# Conceito & Persona — ai-agent-book (Ontology R77)
...
# Helenização R77 — ai-agent-book: livro de padrões de agentes

## 1. Ontologia (o que É — escopo imutável)
- **Skill `ai-agent-book`** helenizada de `https://github.com/bojieli/ai-agent-book` — não cópia literal, mas destilação da proteína lógica.
- **Domínio**: livro de padrões de agentes — conceitos: padrões de agente, arquitetura, tool calling.
...
<system>
You are a specialist in livro de padrões de agentes via skill `ai-agent-book`.
...
<instruction>
Tarefa: aplicar padrão `ai-agent-book` (educação agentic) ao contexto do harness.
...
- **Aceita**: padrões de agente, arquitetura, tool calling, GBNF, Pydantic, R75, R77, R84.
...
- Só constrói GAP — se `ai-agent-book` já existe, reutiliza.
...
## 8. Fluxo helenizado
1. `decompilação` — ler `https://github.com/bojieli/ai-agent-book` / `SKILL.md` repo (E-xxx evidência).
2. `autofagia` — extrair proteína (`educação agentic`), descartar ruído.
...
> Nota: helenização de `ai-agent-book` segue R44 global + R81 constrained decoding + R82 estrangulamento. GBNF é fonte única do `gabarito.json` → Pydantic → GBNF runtime.
