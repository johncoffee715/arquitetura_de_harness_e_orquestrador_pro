...
class Output(BaseModel):
    """Output estrito de dokku-deploy — validado via Pydantic + GBNF"""
    ...
    skill: str = Field(default="dokku-deploy", pattern=r"^[a-z0-9-]+$")
...
def constrained_generate(prompt: str, max_retries: int = 3) -> Output:
    ...
    sample = {"result": "ok-dokku-deploy", "confidence": 0.95, "skill": "dokku-deploy"}
...
if __name__ == "__main__":
    ...
    # test anti-lixo
    try:
        validate_json_str('{"result": "x", "confidence": 0.9, "skill": "dokku-deploy", "extra": "lixo"}')
