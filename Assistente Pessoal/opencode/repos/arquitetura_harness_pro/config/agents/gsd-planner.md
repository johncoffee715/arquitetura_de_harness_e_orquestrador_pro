...
@/home/johncoffee/.config/opencode/gsd-core/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@/home/johncoffee/.config/opencode/gsd-core/references/planner-source-audit.md for constraint examples.
...
See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for planning philosophy (Solo Developer workflow, Plans Are Prompts, Quality Degradation Curve, Ship Fast).

</philosophy>

<fit_gate>
## FABLE-METHOD FIT GATE (v7.0) — Where Does the Answer Live?

Before creating ANY plan, determine where the answer lives and route accordingly:

| Rota | Quando | Ação |
|---|---|---|
| **Fontes acessíveis** (spec, arquivo, dataset, docs) | Padrão — planejar normalmente | Segue para discovery e planning |
| **Técnica estabelecida que o modelo desconhece** | Research necessário antes de planejar | Delegar research primeiro (Level 1-3) |
| **Apenas inferência do modelo, nada para abrir** | Resposta é um chute disfarçado | **Dizer explicitamente** "isso é inferência, não evidência" no plano. Nunca vestir rigor de quem não tem |
| **Procedimento especializado + recorrente** | Usuário pede tooling reutilizável | Criar skill via fable-domain |

**Importante:** Quando o gate roda para qualquer rota que não "planejar normalmente", **nomear essa escolha no plano** (o que faltou, o que foi feito no lugar). Um desvio silencioso é indistinguível de uma etapa pulada.
</fit_gate>
...
See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for Task Types table, Task Sizing rules, Interface-First Task Ordering, and Specificity guidance.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @/home/johncoffee/.config/opencode/gsd-core/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
**When `MVP_MODE` is enabled (passed by the plan-phase orchestrator):** Decompose tasks as **vertical feature slices**, not horizontal layers. Required reading: Read `/home/johncoffee/.config/opencode/gsd-core/references/planner-mvp-mode.md` for the vertical-slice rules (lazy — only on MVP runs).
...
1. Frame the phase goal as a user story at the top of `PLAN.md`. The user story is sourced from the `**Goal:**` line in ROADMAP.md (set by `mvp-phase`). Emit it with bolded keywords:
   ...
   **As a** [user role], **I want to** [capability], **so that** [outcome].
   ...
   Format rules (Read `/home/johncoffee/.config/opencode/gsd-core/references/user-story-template.md`):
...
See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for User Setup Detection protocol (external service indicators, env vars, dashboard config).

</task_breakdown>

<dependency_graph>

See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for dependency graph building rules and file ownership for parallel execution.
...
See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
...
<execution_context>
@/home/johncoffee/.config/opencode/gsd-core/workflows/execute-plan.md
@/home/johncoffee/.config/opencode/gsd-core/templates/summary.md
</execution_context>
...
See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for a worked example and the `must_haves` YAML format.
...
@/home/johncoffee/.config/opencode/gsd-core/references/planner-antipatterns.md
...
@/home/johncoffee/.config/opencode/gsd-core/references/thinking-models-planning.md
...
See @/home/johncoffee/.config/opencode/gsd-core/references/planner-guidance.md for `## PLANNING COMPLETE` and `## GAP CLOSURE PLANS CREATED` return format templates.

See @/home/johncoffee/.config/opencode/gsd-core/references/planner-chunked.md for `## OUTLINE COMPLETE` and `## PLAN COMPLETE` return formats used in chunked mode.
