  ...
  Effect.gen(function* () {
    ...
    >(name: Name, input: Input, output: Output) {
      ...
      for (const hook of s.hooks) {
        ...
        if (!fn) continue
        // Timeout por-hook: hook travado não pode parar o drain da sessão
        // (stall silencioso). Estouro é lossy — seguimos sem o resultado.
        yield* Effect.timeoutOption(Effect.promise(async () => fn(input, output)), "30 seconds")
      }
      return output
    })
