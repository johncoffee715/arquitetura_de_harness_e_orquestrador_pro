...
const registryLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const settleWith = Effect.fn("ToolRegistry.settle")(function* (input: ExecuteInput, advertised?: object) {
      ...
      // CONTEXT.md: failure to retain a Managed Tool Output File does not
      // change a successful tool operation into a failed one — degrade to the
      // unbounded result instead of failing the tool call.
      const bounded = yield* resources
        .bound({ sessionID: input.sessionID, toolCallID: input.call.id, output })
        .pipe(
          Effect.catchTag("ToolOutputStore.StorageError", () =>
            Effect.succeed({ output, outputPaths: [] as string[] }),
          ),
        )
