...
describe("ToolRegistry", () => {
  ...
  it.effect("degrades retention failures into lossy success (CONTEXT.md:194)", () =>
    Effect.gen(function* () {
      const service = yield* ToolRegistry.Service
      yield* service.register({ echo: make() })
      ...
      // Retenção é best-effort: falha de storage não converte uma operação
      // bem-sucedida em erro para o modelo — resultado cru segue, sem paths.
      expect(Exit.isSuccess(exit)).toBe(true)
      if (Exit.isSuccess(exit)) expect("outputPaths" in exit.value).toBe(false)
