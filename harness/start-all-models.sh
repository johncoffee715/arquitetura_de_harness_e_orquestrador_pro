...
declare -A MODELS=(
  [orchestrator]="Ornith-1.5-9B-Q4_K_M.gguf|8083|none|131072|1|gpu"  # FIXO R60: KV@262K=13.95GB+pesos>16GB MI50=OOM; 131072=teto físico seguro (78%)
...
for key in "${KEYS[@]}"; do
  ...
  # Modelos específicos
  case "$FILE" in
    Ornith-1.5*)  ARGS+=(--jinja --temp 0.6 --top-p 0.95 --top-k 20 --chat-template-kwargs '{"enable_thinking": false}') ;;  # sampling oficial unsloth p/ agentic/coding
