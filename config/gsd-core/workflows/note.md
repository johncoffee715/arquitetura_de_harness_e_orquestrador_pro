...
- **Global scope**: `/home/johncoffee/.config/opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/johncoffee/.config/opencode/notes/`)
...
2. Glob `/home/johncoffee/.config/opencode/notes/*.md` (if directory exists) — global notes
...
Global (/home/johncoffee/.config/opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/johncoffee/.config/opencode/notes/` — works in any directory
