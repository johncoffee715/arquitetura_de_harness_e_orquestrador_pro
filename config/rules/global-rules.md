...
- **R70 — PRESERVAÇÃO DA JANELA DO ORQUESTRADOR (guardrail imprescindível)**: o primário NÃO lê, NÃO escreve, NÃO corrige, NÃO faz trabalho pesado — cada token bruto na janela é janela perdida (evidência 24/08: estouro 146K). O primário apenas: **delega, ignita, julga, gerencia, supervisiona, faz self-improvement, self-learning, self-scaffolding**. Trabalho bruto (leitura extensa, escrita, correção iterativa, pesquisa longa) vai SEMPRE para subagentes frescos, que devolvem ao GM apenas evidências e resumos destilados. O GM consome estado compacto (harness_state, orchestrator-metrics, resumos) — nunca a matéria-prima.
