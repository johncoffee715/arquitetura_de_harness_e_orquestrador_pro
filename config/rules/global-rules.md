...
- **R71 — ZERO-TRUST NA EXECUÇÃO (sandbox + gatekeeper de cognição)**: skills/subagentes EXTERNOS ou autofagiados não-confiáveis rodam via `harness/sandbox/secure_runner.sh` (bwrap: sem home, sem rede por default, sem /mnt/dados — flag --harness-ro só p/ leitura; workspace preservado em _done/ p/ auditoria). Escrita na cognição (vault) SOMENTE via `harness/hooks/memory_keeper.sh` (contenção real de path, .md obrigatório, NUL/controles sanitizados, teto 2MB, escrita atômica, log). Defesa por AMBIENTE (jaula sem segredos montados) > filtros regex. Roadmap: eBPF observabilidade → Wasm p/ skills lógicas.
- **R68-b — A2A OBRIGATÓRIO EM COMPLEX/FEATURE (enforcement do R38/R40/R41)**: nenhuma spec, plano ou design é válido sem loop A2A registrado: momus refuta (≥2 rodadas) + artistry gera alternativas + incorporação ANTES da síntese. O primário NÃO sintetiza solo em tasks COMPLEX/FEATURE — o swarm é o processo, não opcional. TRIVIAL/SIMPLE: A2A opcional. Evidência 24/08: qwen3.8-9b sintetizou design solo (sem A2A) → subagentes lazy + spec rasa.
